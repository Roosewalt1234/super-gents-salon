...
const toFormStr = (v: string | null | undefined): string => v ?? "";
const toFormNum = (v: number | null | undefined): string =>
  v != null ? String(v) : "";
...
const EditEmployeeModal = ({
  ...
  /* Fetch branches and employee data when modal opens */
  useEffect(() => {
    if (!open) return;
    ...
    Promise.all([
      ...
      .then(([branchRes, emp]) => {
        ...
        form.reset({
          employee_name: toFormStr(emp.employee_name),
          employee_number: toFormStr(emp.employee_number),
          profile_photo_url: toFormStr(emp.profile_photo_url),
          email: toFormStr(emp.email),
          phone: toFormStr(emp.phone),
          nationality: toFormStr(emp.nationality),
          date_of_birth: toFormStr(emp.date_of_birth),
          gender: toFormStr(emp.gender),
          current_visa_status: toFormStr(emp.current_visa_status),
          current_visa_expiry_date: toFormStr(emp.current_visa_expiry_date),
          notes: toFormStr(emp.notes),
          referred_by: toFormStr(emp.referred_by),
          employment_type: toFormStr(emp.employment_type),
          hire_date: toFormStr(emp.hire_date),
          basic_salary: toFormNum(emp.basic_salary),
          food_allowance: toFormNum(emp.food_allowance),
          ot_amount: toFormNum(emp.ot_amount),
          accommodation_amount: toFormNum(emp.accommodation_amount),
          transport_amount: toFormNum(emp.transport_amount),
          commission_rate: toFormNum(emp.commission_rate),
          position: toFormStr(emp.position),
          assigned_branch_id: toFormStr(emp.assigned_branch_id),
          status: toFormStr(emp.status) || "active",
          visa_branch_id: toFormStr(emp.visa_branch_id),
          visa_expiry_date: toFormStr(emp.visa_expiry_date),
          passport_number: toFormStr(emp.passport_number),
          passport_expiry_date: toFormStr(emp.passport_expiry_date),
          emirates_id_number: toFormStr(emp.emirates_id_number),
          emirates_id_expiry_date: toFormStr(emp.emirates_id_expiry_date),
          ohc_number: toFormStr(emp.ohc_number),
          ohc_expiry_date: toFormStr(emp.ohc_expiry_date),
          iloe_insurance_number: toFormStr(emp.iloe_insurance_number),
          iloe_insurance_expiry_date: toFormStr(emp.iloe_insurance_expiry_date),
          labor_card_number: toFormStr(emp.labor_card_number),
          labor_card_expiry_date: toFormStr(emp.labor_card_expiry_date),
          medical_insurance_number: toFormStr(emp.medical_insurance_number),
          medical_insurance_expiry_date: toFormStr(emp.medical_insurance_expiry_date),
          part_time_card_number: toFormStr(emp.part_time_card_number),
          part_time_card_expiry_date: toFormStr(emp.part_time_card_expiry_date),
        });
      })
      .catch(() => toast.error("Failed to load employee details."))
      .finally(() => setLoadingEmployee(false));
  }, [open, employeeId, tenantId]);
      ...
      <div className="relative z-10 w-full max-w-2xl mx-4 bg-card rounded-2xl shadow-2xl border border-border flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-foreground leading-none">Edit Employee</h2>
              <p className="text-[11px] text-muted-foreground mt-0.5">Update employee details across each section</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          {loadingEmployee ? (
            ...
            <Form {...form}>
              <form id="edit-emp-form" className="space-y-4">
                <Tabs
                  value={activeTab}
